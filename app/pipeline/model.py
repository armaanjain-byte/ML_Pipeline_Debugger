...
class Model:
    ...
    Goals:
    - sklearn compatibility stability
    - deterministic transformed feature ordering
    - robust feature importance extraction
    - safer estimator capability handling
    - reproducible cross-validation behavior
    - stable preprocessing/model integration

    IMPORTANT:
    This implementation preserves the original analytical intent
    and avoids altering recommendation/deployability semantics.
    """
    ...
    def __init__(
        ...
        self._y_train_cache: Optional[pd.Series] = None
    ...
    def train(
        ...
        """
        Builds and trains the pipeline.
        """
        ...
        self.transformed_feature_names_ = self._extract_transformed_feature_names()
    ...
    def predict(self, X_test: pd.DataFrame) -> np.ndarray:
        ...
        if self.pipeline is None:
            raise RuntimeError("Model pipeline has not been trained.")
    ...
    def cross_validate(
        ...
        scoring = self._get_cv_scoring_strategy()

        validation_pipeline = clone(self.pipeline)
        ...
        normalized_scoring_name = scoring.replace("neg_", "")

        return {
            f"cv_mean_{normalized_scoring_name}": float(np.mean(scores)),
            f"cv_std_{normalized_scoring_name}": float(np.std(scores)),
            "cv_folds": int(cv),
        }
    ...
    def feature_importance(self) -> Dict[str, float]:
        ...
        Critical guarantees:
        - transformed feature ordering preserved
        - deterministic name mapping
        - normalized outputs
        - graceful estimator capability handling
        """
        ...
        try:
            ...
            importances = self._extract_importance_values(estimator)

            if importances is None:
                return {}

            transformed_names = self.transformed_feature_names_

            if not transformed_names:
                transformed_names = self._extract_transformed_feature_names()

            if len(transformed_names) != len(importances):
                logger.warning(
                    "Feature importance length mismatch detected. "
                    "Falling back to deterministic generated names."
                )
            ...
            feature_importance_mapping = {
                str(feature_name): float(importance)
    ...
    def _build_pipeline(self, X: pd.DataFrame) -> None:
        """
        Builds a compatibility-safe sklearn pipeline.
        """
        ...
        self._log_high_cardinality_features(X)
    ...
    def _resolve_estimator(self):
        """
        Resolves estimator selection while preserving original behavior.
        """
        ...
        return RandomForestRegressor(**estimator_config)
    ...
    def _extract_importance_values(self, estimator):
        """
        Robust importance extraction hierarchy.
        """
        ...
        return self._compute_permutation_importance(estimator)

    def _compute_permutation_importance(self, estimator):
        """
        Stable permutation-importance fallback.
        """

        if self._X_train_cache is None or self._y_train_cache is None:
            return None
        ...
        try:
            ...
            transformed_X = preprocessor.transform(self._X_train_cache)
    ...
    def _normalize_importances(
        ...
        """
        Prevents unstable visualization scaling.
        """
        ...
        return importances / total_importance

    # ==========================================================
    # Feature Name Extraction
    # ==========================================================

    def _extract_transformed_feature_names(self) -> List[str]:
        """
        Deterministic transformed feature extraction.
        ...
        try:
            ...
            for transformer_name, transformer, columns in preprocessor.transformers_:
                ...
                if transformer_name == "num":
                    extracted_names.extend(column_list)
    ...
    def _extract_categorical_feature_names(
        ...
        """
        sklearn-version-safe categorical feature extraction.
        """
        ...
        try:
            ...
            for column_name, category_values in zip(columns, categories):
    ...
    def _get_cv_scoring_strategy(self) -> str:
        if self.task_type == "classification":
            return "f1_weighted"
    ...
    def _log_high_cardinality_features(
        ...
        """
        Preserves existing observability behavior.
        """

        for column_name in self.categorical_features_:
